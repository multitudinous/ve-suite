...
performSave()
{
    ...
    // Save physics state.
    osgDB::writeObjectFile( ps, "physics_state_sr2.sgb" );
...
performRestore()
{
    ...
    // Load physics state
    osg::ref_ptr< osg::Object > objIn = osgDB::readObjectFile( "physics_state_sr2.sgb" );
    osgbBullet::PhysicsState* ps = static_cast< osgbBullet::PhysicsState* >( objIn.get() );
    if( ps == NULL )
    {
        osg::notify( osg::FATAL ) << "performRestore: Error restoring file physics_state_sr2.sgb" << std::endl;
        return;
    }
    ...
    for( it = dm.begin(); it != dm.end(); it++ )
    {
        ...
        EnablePhysicsForRefID epr( rid.get(), it->second.get() );
        _root->accept( epr );
    }
