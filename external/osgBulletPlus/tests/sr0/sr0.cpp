...
#include <osgTools/FindNamedNode.h>
...
//#define DBG_DRAW
#ifdef DBG_DRAW
osgBullet::GLDebugDrawer* _dbgDraw( NULL );
#endif
...
osgBullet::PhysicsState ps;
static unsigned int id( 0 );
static osgBulletPlus::RestorePhysics restoreHandler;
...
{
    osgTools::FindNamedNode fnn( nodeName );
    root->accept( fnn );
    if( fnn._napl.empty() )
    {
    ...
    osg::ref_ptr< osgTools::AbsoluteModelTransform > model( new osgTools::AbsoluteModelTransform );
    model->setDataVariance( osg::Object::DYNAMIC );
    osgTools::insertAbove( node, model );
    ...
    osg::ref_ptr< osgBullet::PhysicsData > pd = new osgBullet::PhysicsData;
    pd->_cr = converter.getOrCreateCreationRecord();
    pd->_body = rb;
    std::ostringstream ostr;
    ostr << "id" << id++;
    osg::ref_ptr< osgTools::RefID > rid = new osgTools::RefID( ostr.str() );
        ...
        switch( ea.getEventType() )
        {
            case osgGA::GUIEventAdapter::KEYDOWN:
            {
                if( ea.getKey() == 's' )
                {
                    ...
                    osgBulletPlus::savePhysics( "sr0_test", _root.get(), ps );
...
#ifdef DBG_DRAW
                    _dbgDraw = new osgBullet::GLDebugDrawer();
                    _dbgDraw->setDebugMode( ~btIDebugDraw::DBG_DrawText );
                    bulletWorld->setDebugDrawer( _dbgDraw );
                    _root->addChild( _dbgDraw->getSceneGraph() );
#endif

                    restoreHandler.restore( "sr0_test", _root.get(), bulletWorld );
      ...
      char ** argv )
{
    ...
    {
        bool unlock( false );
        if( restoreHandler.status() != osgBulletPlus::RestorePhysics::RESTORE_IN_PROGRESS )
        {
        ...
        else if( restoreHandler.status() == osgBulletPlus::RestorePhysics::RESTORE_IN_PROGRESS )
        {
