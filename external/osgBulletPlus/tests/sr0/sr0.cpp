...
enablePhysics( osg::Node* root, const std::string& nodeName, btDynamicsWorld* bw )
{
    ...
    osg::ref_ptr< osg::Group > copyGrp;
    osg::Group* asGrp = node->asGroup();
    if( asGrp == NULL )
    {
        copyGrp = new osg::Group;
        copyGrp->addChild( new osg::Node( *node, osg::CopyOp::DEEP_COPY_ALL ) );
    }
    else
        copyGrp = new osg::Group( *asGrp, osg::CopyOp::DEEP_COPY_ALL );
        ...
        switch( ea.getEventType() )
        {
            case osgGA::GUIEventAdapter::KEYDOWN:
            {
                ...
                else if( ea.getKey() == 'r' )
                {
                    ...
                    osgbBulletPlus::RestorePhysics::RestoreStatus stat =
                        restoreHandler.restore( "sr0_test", bulletWorld, _root.get() );
                    if( stat != osgbBulletPlus::RestorePhysics::RESTORE_IN_PROGRESS )
                        osg::notify( osg::ALWAYS ) << "Unable to restore physics." << std::endl;
      ...
      char ** argv )
{
    ...
    {
        bool unlock( false );
        osgbBulletPlus::RestorePhysics::RestoreStatus stat = restoreHandler.status();
        if( stat != osgbBulletPlus::RestorePhysics::RESTORE_IN_PROGRESS )
        {
        ...
        else if( stat == osgbBulletPlus::RestorePhysics::RESTORE_IN_PROGRESS )
        {
