...
{
public:
    InteractionManipulator( btDiscreteDynamicsWorld* world, osg::Group* sg, osgBullet::PhysicsThread* pt=NULL )
      : _world( world ),
        _sg( sg ),
        _pt( pt )
    {}
...
makeDoor( btDiscreteDynamicsWorld* bw, InteractionManipulator* im )
{
    ...
    osgBullet::MotionState* ms = static_cast< osgBullet::MotionState* >( rb->getMotionState() );
      ...
      char ** argv )
{
    ...
    root->addChild( osgBullet::generateGroundPlane( osg::Vec4( 0.f, .2f, 1.f, -1.f ), bulletWorld ) );
