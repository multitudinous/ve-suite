...
#include "osgBulletPlus/OSGToCollada.h"
...
#include <osgTools/GeometryModifier.h>
#include <osgTools/DecimatorOp.h>
#include <osgTools/SimplifierOp.h>
#include <osgBulletPlus/VertexAggOp.h>
#include <osgTools/ReducerOp.h>
...
public:
    ProcessSceneGraph( const bool overall, const std::string& nodeName,
        const BroadphaseNativeTypes shapeType, const osgBullet::AXIS axis )
...
OSGToCollada::OSGToCollada( osgBullet::CreationRecord& cr )
{
    _cr = new osgBullet::CreationRecord( cr );
}
OSGToCollada::~OSGToCollada()
{
}
...
bool OSGToCollada::convert( const std::string& outputFileName )
{
    ...
    if ( getSimplifyPercent() != 1.f )
    {
        ...
        osgTools::SimplifierOp* simplify = new osgTools::SimplifierOp;
        simplify->_simplifier->setSampleRatio( getSimplifyPercent() );
        osgTools::GeometryModifier gm( simplify );
        root->accept( gm );
        gm.displayStatistics( osg::notify( osg::INFO ) );
    }
    ...
    if ( maxVerts > 0 )
    {
        ...
        osgBulletPlus::VertexAggOp* aggregate = new osgBulletPlus::VertexAggOp;
        aggregate->setMaxVertsPerCell( maxVerts );
        aggregate->setMinCellSize( cellSize );
        osgTools::GeometryModifier gm( aggregate );
        root->accept( gm );
        gm.displayStatistics( osg::notify( osg::INFO ) );
    }
        ...
        (maxEdgeError < 360.f) )
    {
        ...
        osgTools::ReducerOp* reduce = new osgTools::ReducerOp;
        reduce->setGroupThreshold( groupThreshold );
        reduce->setMaxEdgeError( maxEdgeError );
        osgTools::GeometryModifier gm( reduce );
        root->accept( gm );
        gm.displayStatistics( osg::notify( osg::INFO ) );
    }
...
osgBullet::CreationRecord*
OSGToCollada::getOrCreateCreationRecord()
{
    if( !_cr.valid() )
        _cr = new osgBullet::CreationRecord;
    return( _cr.get() );
}
const osgBullet::CreationRecord*
