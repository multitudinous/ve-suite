...
public:
    ...
    void apply( osg::Group& node )
    {
        ...
        if( shape != NULL )
        {
            ...
            osg::Node* debugNode = osgbBullet::osgNodeFromBtCollisionShape( shape );
...
protected:
    ...
    static btCollisionShape* createChildCollisionShapes( osg::BoundingBox& bbox, osg::Group& node, bool palm )
    {
        ...
        return osgbBullet::btBoxCollisionShapeFromOSG( tempRoot.get(), &bbox );
    }
...
void HandNode::updateTransform()
{
...

    {
        ...
        btTransform btm( osgbBullet::asBtTransform( m2 ) );
        if( _motion != NULL )
        {
            // TBD pause/unpause around this.
...
void HandNode::registerMultiThreaded( osgbBullet::PhysicsThread* pt )
{
    _pt = pt;
}
...
void HandNode::ArticulationInfo::setBulletTransform()
{
    if( _btChildIdx >= 0 )
    {
        ...
        _cs->updateChildTransform( _btChildIdx, osgbBullet::asBtTransform( btm ) );
