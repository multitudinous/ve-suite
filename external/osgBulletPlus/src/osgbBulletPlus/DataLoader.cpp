...
DataLoader::DataLoader( osg::Transform* attachPoint, const std::string& fileName, const osgbBullet::PhysicsData* pd, btDynamicsWorld* bw )
  : _attachPoint( attachPoint ),
    ...
    _cpt( NULL )
{
    amt = dynamic_cast< osgwTools::AbsoluteModelTransform* >( attachPoint );
    commonInit();
}

DataLoader::DataLoader( osg::Node* loadedModel, const osgbBullet::PhysicsData* pd, btDynamicsWorld* bw )
  : _fileName( "" ),
    _pd( pd ),
    _bw( bw ),
...
DataLoader::commonInit()
{
    ...
    // Error check. osgbBullet only supports attach points that are
    ...
    bool finished( false );
    {
        ...
        case DataLoader::LOAD_MODEL:
        {
            if( ( _lnt != NULL ) && ( !_lnt->isRunning() ) )
            {
                if( _lnt->_node.get() != NULL )
                {
                    ...
                    osgwTools::AbsoluteModelTransform* amt = dynamic_cast< osgwTools::AbsoluteModelTransform* >( _attachPoint.get() );
                    if( amt != NULL )
                    {
        ...
        case DataLoader::COMPLETE:
        {
            if( ( _cpt != NULL ) && ( !_cpt->isRunning() ) )
            {
                ...
                osgbBullet::MotionState* ms = dynamic_cast< osgbBullet::MotionState* >( rb->getMotionState() );
