...
savePhysics( const std::string baseName, const osg::Node* root, const osgbBullet::PhysicsState& ps )
{
    ...
    // Save physics state.
    const std::string osgbName( baseName + std::string( ".sgb" ) );
    result = osgDB::writeObjectFile( ps, osgbName );
    if( !result )
    {
        osg::notify( osg::FATAL ) << "savePhysics: Failed to write file \"" << osgbName << "\"." << std::endl;
        return false;
    }
    ...
    if( !result )
    {
        osg::notify( osg::FATAL ) << "savePhysics: Failed to write file \"" << skelName << "\"." << std::endl;
        return false;
    }
...
public:
    ...
    void run()
    {
        ...
        if( skel == NULL )
        {
            osg::notify( osg::FATAL ) << "RestorePhysics: Failed to read file \"" << _fileName << "\"." << std::endl;
            return;
        }
...
RestorePhysics::restore( const std::string baseName, btDynamicsWorld* bw, osg::Node* root )
{
    ...
    _root = root;
    _bw = bw;
    _osgbName = std::string( baseName + std::string( ".sgb" ) );
