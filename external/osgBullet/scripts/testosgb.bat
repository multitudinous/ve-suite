...
echo.
echo Test reading collada files
echo Valid keys are 0, 1, and 2.
echo.
colladaread USMC23_1020.ASM.ive

echo.
echo.
echo.
echo Hand interaction tests.
echo Try to lift the concave object from its center.
echo.
CadDemo concave.txt
echo.
echo Hand interaction tests.
echo Still need to hook up a hinge constraint.
echo The door will fall over in this test.
echo.
CadDemo door.txt
echo.
echo Hand interaction tests.
echo Get under the jack and move it onto a shelf.
echo Seems to be tricky now that we've converted the jack to a convexHull.
echo.
CadDemo shelf.txt
echo.
echo Hand interaction tests.
echo Still need to hook up a slider constraint.
echo Way too many contact points.
echo.
CadDemo drawer.txt
