...
{
    supportsExtension( "osgb", "osgBullet physics simulation state." );
...
{
    return "osgBullet physics simulation state";
}
...
ReaderWriterOSGB::readObject( const std::string& fileName, const Options* options ) const
{
    ...
    osg::ref_ptr< osgBullet::PhysicsState > ps = new osgBullet::PhysicsState;
    unsigned int idx;
    for( idx = 0; idx < numEntries; idx++ )
    {
        osg::notify( osg::INFO ) << "OSGB: Reading entry " << idx << std::endl;

        osg::ref_ptr< osgTools::RefID > rid = static_cast< osgTools::RefID* >( fr.readObject() );
        if( rid == NULL )
        {
            osg::notify( osg::INFO ) << "OSGB: Failed, rid " << rid.get() << std::endl;
            return( osgDB::ReaderWriter::ReadResult::ERROR_IN_READING_FILE );
        }
        osg::ref_ptr< osgBullet::PhysicsData > pd = static_cast< osgBullet::PhysicsData* >( fr.readObject() );
        if( pd == NULL )
        {
...
ReaderWriterOSGB::writeObject( const osg::Object& obj, const std::string& fileName, const Options* options ) const
{
    ...
    const osgBullet::PhysicsState* ps = dynamic_cast< const osgBullet::PhysicsState* > ( &obj );
    if( ps == NULL )
        return( osgDB::ReaderWriter::WriteResult::ERROR_IN_WRITING_FILE );


    const osgBullet::PhysicsState::DataMap& dm = ps->getDataMap();
    fw << "Physics data entries " << dm.size() << std::endl;

    osgBullet::PhysicsState::DataMap::const_iterator it;
    for( it = dm.begin(); it != dm.end(); it++ )
    {
        osg::ref_ptr< osgTools::RefID > rid = new osgTools::RefID( it->first );
        fw.writeObject( *rid );
