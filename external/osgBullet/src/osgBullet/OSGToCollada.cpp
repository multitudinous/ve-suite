...
bool OSGToCollada::convert( const std::string& outputFileName )
{
    ...
    if ( _decimatorPercent < 1.f )
    {
        ...
        decimate->setIgnoreBoundries(_decimatorIgnoreBoundries);
...
void
OSGToCollada::setDecimateParamaters( float decimatePercent, float maxError, bool decimateIgnoreBoundries )
{
    _decimatorPercent = decimatePercent;
    _decimatorMaxError = maxError;
    _decimatorIgnoreBoundries = decimateIgnoreBoundries;
}
