...
public:
    ...
    HalfEdgeCollapse(){_ignoreBoundries = false;};
    ~HalfEdgeCollapse();
    ...
    void updateErrorMetricForEdge(Edge* edge)
    {
        ...
        if (edge->getMaxNormalDeviationOnHalfEdgeCollapse()<=1.0 && (!edge->isAdjacentToBoundary() || getIgnoreBoundries() )
                                                                 && !edge->concavityFlip()     )
...
DecimatorOp::DecimatorOp( const DecimatorOp& rhs, const osg::CopyOp& copyOp )
{
    ...
    _ignoreBoundries = rhs._ignoreBoundries;
}
...
void DecimatorOp::decimate(osg::Geometry& geometry)
{
    ...
    hec.setIgnoreBoundries(this->_ignoreBoundries);
    ...
    {
        osg::notify(osg::INFO)<<std::endl<<"Decimator, in = "<<numOriginalPrimitives<<"\tout = "<<hec._triangleSet.size()<<"\terror="<<(*hec._edgeSet.begin())->getErrorMetric()<<"\tvs "<<getMaximumError()<<std::endl<<std::endl;
