    ...
    bool handle( const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& )
    {
        switch( ea.getEventType() )
        {
            case osgGA::GUIEventAdapter::KEYUP:
            {
                ...
                osgTools::AbsoluteModelTransform* amt = dynamic_cast< osgTools::AbsoluteModelTransform* >( _n.get() );
                if( amt != NULL ) {
                    initialTrans = amt->getMatrix();
                    amt->setMatrix( osg::Matrix::identity() );
                }

                osgBullet::RefRigidBody* rb = dynamic_cast< osgBullet::RefRigidBody* >( _n->getUserData() );
                if( rb != NULL )
                {
                ...
                if( !( osgBullet::loadDae( _n.get(), np, daeName, _dw ) ) )
                    return false;
          ...
          char* argv[] )
{
    ...
    osg::ref_ptr< osgTools::AbsoluteModelTransform > model =
        new osgTools::AbsoluteModelTransform;
