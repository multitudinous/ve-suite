      ...
      char ** argv )
{
    ...
    for( itr = crw->_nodeDaeMap.begin(); itr != crw->_nodeDaeMap.end(); itr++ )
    {
        ...
        for( idx=0; idx<fnn._napl.size(); idx++ )
        {
            ...
            osgBullet::RigidBody* rb = new osgBullet::RigidBody( osgBullet::loadDae( amt, np, itr->second ) );
    ...
    for( hitr = crw->_hingeList.begin(); hitr != crw->_hingeList.end(); hitr++ )
    {
        ...
        FindNamedNode fnnA( hi._nodeA );
        orient->accept( fnnA );
        FindNamedNode fnnB( hi._nodeB );
        orient->accept( fnnB );

        FindNamedNode::NodeAndPath& napA( fnnA._napl[ 0 ] );
        osgBullet::AbsoluteModelTransform* amtA = dynamic_cast< osgBullet::AbsoluteModelTransform* >( napA.first->getParent( 0 ) );
        if( amtA == NULL )
        {
            osg::notify( osg::FATAL ) << "Hinge node (" << hi._nodeA << ") parent is not AMY." << std::endl;
            continue;
        }
        osgBullet::RigidBody* rbA = dynamic_cast< osgBullet::RigidBody* >( amtA->getUserData() );
        if( rbA == NULL )
        {
        ...
        FindNamedNode::NodeAndPath& napB( fnnB._napl[ 0 ] );
        osgBullet::AbsoluteModelTransform* amtB = dynamic_cast< osgBullet::AbsoluteModelTransform* >( napB.first->getParent( 0 ) );
        if( amtB == NULL )
        {
            osg::notify( osg::FATAL ) << "Hinge node (" << hi._nodeB << ") parent is not AMY." << std::endl;
            continue;
        }
        osgBullet::RigidBody* rbB = dynamic_cast< osgBullet::RigidBody* >( amtB->getUserData() );
        if( rbB == NULL )
        {
        ...
        const btVector3 btPivotA( osgBullet::asBtVector3( hi._pivotA ) );
        const btVector3 btPivotB( osgBullet::asBtVector3( hi._pivotB ) );
        btVector3 btAxisA( osgBullet::asBtVector3( hi._axisA ) );
        btVector3 btAxisB( osgBullet::asBtVector3( hi._axisB ) );
    ...
    for( sitr = crw->_sliderList.begin(); sitr != crw->_sliderList.end(); sitr++ )
    {
        ...
        m = m * axisMatrix;
        btMatrix3x3 m3A( osgBullet::asBtMatrix3x3( m ) );
        ...
        {
            FindNamedNode::NodeAndPath& nap( fnnB._napl[ idx ] );
            osgBullet::AbsoluteModelTransform* amtB = dynamic_cast< osgBullet::AbsoluteModelTransform* >( nap.first->getParent( 0 ) );
            if( amtB == NULL )
            {
            ...
            bs = amtB->getBound();
            osg::Vec3 centerB = bs.center();
            btVector3 offsetA( osgBullet::asBtVector3( centerB - centerA ) );

            // Get the 3x3 basis of body B's reference frame.
            amtB->computeLocalToWorldMatrix( m, NULL );
            m = m * axisMatrix;
            btMatrix3x3 m3B( osgBullet::asBtMatrix3x3( m ) );
