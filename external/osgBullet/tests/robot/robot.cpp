    ...
    void apply( osg::Geode& node )
    {
        ...
        for ( idx=0; idx < node.getNumDrawables(); idx++ )
        {
            ...
            if (shape)
            {
                ...
                if (bsv._shape)
                {
                    osgBullet::RefCollisionShape* collision = new osgBullet::RefCollisionShape( bsv._shape );
    ...
    btCollisionShape* accumulateChildCollisionShapes( osg::Group& node )
    {
        ...
        for ( idx=0; idx < node.getNumChildren(); idx++ )
        {
            ...
            if ( dynamic_cast< osg::MatrixTransform* >( child ) )
                continue;
            osgBullet::RefCollisionShape* bcs = dynamic_cast< osgBullet::RefCollisionShape* >( child->getUserData() );
            if (!bcs)
                continue;
