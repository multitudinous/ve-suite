...
// oqTest00
//
// This test code demonstrates inserting OcclusionQueryNodes manually into
//   a scene graph.
...
// Ptr to geom that we'll modify.
static osg::ref_ptr<osg::Geometry> g_geom;
static osg::Vec3 g_center( -1.5f, .5f, 1.f );
static float g_radius( 1. );
            ...
            case(osgGA::GUIEventAdapter::KEYUP):
            {
                if (ea.getKey()==osgGA::GUIEventAdapter::KEY_F4)
                {
                    if (g_radius == 1.f )
                        ...
                        g_radius = 1.f;
                    setOctohedronVertices( *g_geom, g_center, g_radius );
...
createScene()
{
    ...
    g_geom = createOctohedron(
        g_center, g_radius, osg::Vec4( 1.f, .5f, 1.f, 1.f ) );
    geode->addDrawable( g_geom.get() );

    osg::ref_ptr<osg::Geometry> draw = createOctohedron(
        osg::Vec3( 1.5f, -1.f, .5f ), 1.f, osg::Vec4( 1.f, 1.f, 1.f, 1.f ) );
    geode->addDrawable( draw.get() );

    osgOQ::OcclusionQueryNode* oqn = new osgOQ::OcclusionQueryNode;
    oqn->addChild( geode );
    grp->addChild( oqn );
