...
starReader::starReader( std::string paramFile )
{
	...
	this->scaleIndex = 0;      // scaleindex default is no scaling
...
vtkUnstructuredGrid * starReader::GetUnsGrid( void )
{
	...
	while( !feof(fcells) )
	{
		fscanf(fcells,"%d %d %d %d %d %d %d %d %d %d %d\n", 
			&cId, 
			&cPt[0], &cPt[1], &cPt[2], &cPt[3], &cPt[4], &cPt[5], &cPt[6], 
			&cPt[7], &cGroup, &cType); 

		numStarCells++;
		numVtkCells++;
	}
	std::cout << "\tTotal no. of cells in star-cd model  = " 
		<< numStarCells << std::endl;
	std::cout << "\tTotal no. of vtk cells to be created = " 
		<< numVtkCells << std::endl;
	...
	{
		fscanf(fcells,"%d %d %d %d %d %d %d %d %d %d %d\n", 
			&cId, &cPt[0], &cPt[1], &cPt[2], &cPt[3], &cPt[4], &cPt[5], 
			&cPt[6], &cPt[7], &cGroup, &cType);
	...
	if ( this->numVectors > 0 )
	{
		...
		scalarData[ this->numScalars ]->SetNumberOfTuples( 
			maxOrigVertexId-vShift+1 );
	}
	...
	if ( vec != NULL )
	{
		...
		letUsersAddParamsToField( this->numScalars + 1, 
			scalarData, uGridPointData, 0 );
	...
	{
		letUsersAddParamsToField( this->numScalars, scalarData,
			uGridPointData, 0 );
		for (i=0; i < this->numScalars; i++) 
			scalarData[ i ]->Delete();
	}
...
void starReader::ReadParameterFile( void )
{
	...
	while( 1 )
	{
		...
		else if( tagName.compare( "SCALEINDEX" )==0 )
		{
			...
			if ( this->debug )
			{
				std::cout << "scale selection = " << 
					this->scaleIndex << std::endl;
			}
		}
	...
	else if ( (this->scaleIndex != 1 || this->scaleIndex !=0) 
		&& scaleFactorSpecified )
	{
		std::cout << "\n!!! Indeterminate case -- "
