...
float SampleAlpha(sampler2D Map, vec2 MapCoord)
{
return(texture2D( Map, MapCoord ).a);
}

vec3 SampleRGB(sampler2D Map, vec2 MapCoord)
{
return(texture2D( Map, MapCoord ).rgb);
}
...
vec3 MapAlphaToNormal(sampler2D Map, vec2 MapCoord, float scale)
{
    float BaseAmplitude = SampleAlpha(Map, MapCoord);  // alpha at txcoord point

    vec2  DeltaAmplitudeXY = 
        vec2(SampleAlpha(Map, MapCoord + vec2(localDFD.x, 0.0)),
             SampleAlpha(Map, MapCoord + vec2(0.0, localDFD.y)));
...
varying vec3 ViewDirection; // for specular
...
uniform float TargetMin;
uniform float TargetMax;


void main( void )
{
   vec2  TexScale = vec2(fScale, fScale);
   localDFD.x = 0.002; // dFdx(TexcoordLayerZero);
   localDFD.y = 0.002; // dFdy(TexcoordLayerZero);
   //localDFD *= TexScale;
   vec2  TexCoordScaled = TexcoordLayerZero * TexScale;
   vec3  fvLightDirection = normalize( LightDirection );
   
   //float fMacroGrassColor = clampedfBmZeroToOne(100.0 + (TexCoordScaled * fMacroToMicroScale), 2.0); // 100: random offset
   vec4  fvBaseColor     = vec4(SampleRGB(bgTexture, TexCoordScaled), 1.0);
   float fPaintValue      = SampleAlpha(accumTexture, TexCoordScaled);
   vec4  fvPaintedColor   = mix(fvBaseColor, vec4(_surfaceColor1, 1.0), clamp(fPaintValue * 2.0, 0.0, 1.0));
   //fvPaintedColor   = vec4(_surfaceColor1, 1.0);

   if(_AccumEnabled1 > 0.0)
   {
	vec4 loBlue = vec4(0.0, 0.0, 0.5, 1.0);
	vec4 hiBlue = vec4(0.0, 0.0, 1.0, 1.0);
	vec4 loGreen = vec4(0.0, 0.5, 0.0, 1.0);
	vec4 hiGreen = vec4(0.0, 1.0, 0.0, 1.0);
	vec4 loRed = vec4(0.5, 0.0, 0.0, 1.0);
	vec4 hiRed = vec4(1.0, 0.0, 0.0, 1.0);

   float TargetMinOver255 = (TargetMin / 255.0);
   float TargetMaxOver255 = (TargetMax / 255.0);
   float TargetRngOver255 = ((TargetMax - TargetMin) / 255.0);
   if(fPaintValue == 0.0)
	   {
	   gl_FragColor = fvBaseColor; // no paint applied
	   } 
   else if(fPaintValue < TargetMinOver255) // under minimum threshold, show blue shades
	   {
	   float factor = fPaintValue / TargetMinOver255; // range is 0,TargetMin
	   gl_FragColor = mix(loBlue, hiBlue, factor);
	   } 
   else if(fPaintValue < (TargetMax / 255.0)) // in goldilocks zone, show green shades
	   {
	   float factor = (fPaintValue - TargetMinOver255) / TargetRngOver255; // range is TargetMin,TargetMax
	   gl_FragColor = mix(loGreen, hiGreen, factor);
	   } 
   else // over threshold, show red shades
	   {
	   float factor = (fPaintValue - TargetMaxOver255) / (255.0 - TargetMaxOver255);
	   gl_FragColor = mix(loRed, hiRed, factor);
	   } 
   } // if
   else
   {   
	   vec3  fvNormal         = MapAlphaToNormal(accumTexture, TexCoordScaled, 0.0001);
	   ...
	   vec4  fvTotalAmbient   = fvAmbient * fvPaintedColor;
	   vec4  fvTotalDiffuse   = fvDiffuse * fNDotL * fvPaintedColor;
