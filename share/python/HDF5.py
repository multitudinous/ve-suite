...
class HDF5(SConsAddons.Options.PackageOption):
   """
   ...
   def __init__(self, name, requiredVersion, required=True,
      ...
      """
      help_text = "Base directory for HDF5 (Requires PYTHONPATH to be set to <vesuitehome>/share/python)."
      self.baseDirKey = "HDF5BaseDir"
      ...
      self.baseDir = None
      if libList == None:
         self.libList=['hdf5', 'hdf5_cpp', 'hdf5_hl','sz']
      else:
   ...
   def startProcess(self):
      print "Checking for HDF5...",

   def setInitial(self, optDict):
      " Set initial values from given dict "
      if self.verbose:
         print "   Applying initial HDF5 settings."
   ...
   def validate(self, env):
      ...
      # Check that an include file (include/hdf5.h) exists
      # Update the temps for later usage
      passed = False
      self.available = False

      if self.baseDir is None:
         self.checkRequired("HDF5 base dir (HDF5BaseDir) was not specified")
         return

      if not os.path.isdir(self.baseDir):
         self.checkRequired("HDF5 base dir %s does not exist" % self.baseDir)
         return

      hdf5_version_file = pj(self.baseDir, 'include', 'hdf5.h')
      if not os.path.isfile(hdf5_version_file):
         self.checkRequired("%s not found" % hdf5_version_file)
         return
      else:
         passed = True
   ...
   def apply(self, env, optimize = False, useCppPath = False):
      ...
      """
      if self.baseDir is None:
         self.checkRequired("OSG base dir (HDF5BaseDir) was not specified")
         return
   ...
   def dumpSettings(self):
      ...
      print "HDF5BaseDir:", self.baseDir
