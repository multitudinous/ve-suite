...
class HDF4(SConsAddons.Options.PackageOption):
   """
   ...
   def __init__(self, name, requiredVersion, required=True,
      ...
      """
      help_text = "Base directory for HDF4 (Requires PYTHONPATH to be set to <vesuitehome>/share/python)."
      self.baseDirKey = "HDF4BaseDir"
   ...
   def startProcess(self):
      print "Checking for HDF4...",

   def setInitial(self, optDict):
      " Set initial values from given dict "
      if self.verbose:
         print "   Applying initial HDF4 settings."
   ...
   def validate(self, env):
      ...
      # Check that an include file (include/hdf.h) exists
      # Update the temps for later usage
      passed = False
      self.available = False

      if self.baseDir is None:
         self.checkRequired("HDF4 base dir (HDF4BaseDir) was not specified")
         return
      ...
      hdf4_version_file = pj(self.baseDir, 'include', 'hdf.h')
      if not os.path.isfile(hdf4_version_file):
         self.checkRequired("%s not found" % hdf4_version_file)
         return
      else:
         passed = True
   ...
   def dumpSettings(self):
      ...
      print "HDF4BaseDir:", self.baseDir
