 ...
 *
 * -----------------------------------------------------------------
 * File:          $RCSfile: UnstructuredGridWriter.cpp,v $
 * Date modified: $Date$
 * Version:       $Rev$
 * -----------------------------------------------------------------
 *
...
vtkUnstructuredGrid * convertToUnstructuredGrid(vtkDataSet *rGrid) {
   ...
   //cout << "Rgrid number of cells " << numCells << endl;
   //cout << "Rgrid number of points " << numPts << endl;
...
// Set up array to hold the ghost free data
   ...
   for (int paramIndex=0; paramIndex<numArrays; paramIndex++) {
         ...
         for (int k=0; k<resH->kMax2; k++) {
            for (int j=0; j<resH->jMax2; j++) {
               for (int i=0; i<resH->iMax2; i++) {
                  ...
                  if (((resH->kMax2!=1)&&(k==0 || k==resH->kMax2-1 || j==0 || j==resH->jMax2-1 || i==0 || i==resH->iMax2-1))
                       || ((resH->kMax2==1)&& (j==0 || j==resH->jMax2-1 || i==0 || i==resH->iMax2-1)))
                     index++;
                  else {
                     // Void fraction
                     if (varNum==0) {
      ...
      //  cout << "Index = " << bb << "  Scalar Data = " << scalarData[bb] << endl;
      //}

      //cout << "Counter = " << counter << endl;
                //cout << "Parmindex = " << paramIndex << endl;
...
int UnstructuredGridWriter(char *inFile, char *outDir, int tStep, int maxTs, int type, resHead *resH, spHead *spH, mfixData *mfD) {
   ...
   if ((!strncmp(resH->coordinates, "CYLINDRICAL", 11))  && (resH->kMax2 != 1)) {
      ...
      hCell->Delete();
      // Attach point information to the unstructured grid
      cout << "Before SetPoints" << endl;
      uGrid->SetPoints(uPoints);
      // Attach cell information to the unstructured grid
      cout << "Before SetCells" << endl;
      uGrid->SetCells(12, hexas);
//      hCell->Delete();
      // Attach point information to the unstructured grid
//      cout << "Before SetPoints" << endl;
//      uGrid->SetPoints(uPoints);
...
//      uGrid->SetCells(cellTypePtr, hexas);
   ...
   // Handle cartesian coordinates
   else if ((!strncmp(resH->coordinates, "CARTESIAN", 9)) || (resH->kMax2 == 1)){
      ...
      for (int i=0; i<resH->kMax2; i++) {
         zCoords->SetTuple1(i, dz);
         dz += resH->dz[i];
      }
      // Set up the rectilinear grid
      if(resH->kMax2 == 1 ){
        rGrid->SetDimensions(resH->iMax2-1,resH->jMax2-1, 1);
      }
      else{
   ...
   if ((!strncmp(resH->coordinates, "CARTESIAN", 9))|| (resH->kMax2 == 1)){
      //cout << " Before converted to ugrid " << endl;
      uGrid = convertToUnstructuredGrid(rGrid);
   }

   //cout << " Converted to ugrid " << endl;

   //cout << " doGhost = " << doGhost << endl;

   // Remove ghost cells
   if (doGhost)
      filteredGrid = filterMeshByCellData(uGrid);

   //cout << "After filter grid" << endl;
