...
//      for ( uGrid->GetCells()->InitTraversal(); uGrid->GetCells()->GetNextCell(npts,pts); )
   ...
   {
      cout << "can only use this function on vtkUnstructuredGrid or vtkPolyData" << endl;
   ...
   for (j=0; j < numVertices; j++)
   {
      if ( isNeededPoint[j] ) 
      {
         grid->GetPoints()->GetPoint( j, vertex );      // get coordinates of the vertex
         smallVertices->InsertPoint( k, vertex );         // insert this vertex
   ...
   if ( numNeededVertices < numVertices ) 
   {
      ...
      if ( grid->IsA("vtkUnstructuredGrid") )
      {
         ...
         //vtkUnstructuredGrid *smallGrid = vtkUnstructuredGrid::New();
...
*/
   ...
   {
      //cout << "can only use this function on vtkUnstructuredGrid or vtkPolyData" << endl;
      cout << "can only use this function on vtkUnstructuredGrids" << endl;
      delete [] isNeededPoint;
      return;
   }
   ...
   if ( numFieldArrays )
   {
      ...
      for (int i=0; i<grid->GetPointData()->GetNumberOfArrays(); i++ )
      {
         ...
         // vectors won't be written in field data
         if ( numComponents > 1 )
            continue;
