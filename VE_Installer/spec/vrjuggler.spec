BuildRequires: aaa_base acl attr audiere audiere-devel autoconf automake bash binutils bison boost boost-devel bzip2 coreutils cpio cpp cppdom cppdom-devel devs diffutils doxygen e2fsprogs e2fsprogs-devel expat file filesystem findutils gawk gcc gcc-c++ gettext glibc glibc-devel glibc-locale gmtl grep gzip info java-1_4_2-sun java-1_4_2-sun-devel java-1_4_2-sun-plugin klogd libacl libattr libgcc libstdc++ libstdc++-devel m4 make man mktemp module-init-tools openal openal-devel patch permissions popt procinfo procps psmisc pwdutils python python-devel readline sed sysvinit tar timezone unzip update-alternatives update-desktop-files util-linux xorg-x11-Mesa xorg-x11-Mesa-devel xorg-x11-devel xorg-x11-libs zlib zlib-devel  
...
./configure.pl --with-boost=%{_prefix} \
               --with-boost-includes=%{_prefix}/include \
               --with-gmtl=%{_prefix} \
               --with-audiere=%{_prefix} \
               --prefix=%{_prefix} %{abi_option}

%build
export PATH=${PATH}:/usr/lib/jvm/java/bin
make %{?_smp_mflags} BUILD_PROF_C=N BUILD_PROF_CXX=N buildworld

%install
make BUILD_PROF_C=N BUILD_PROF_CXX=N install prefix=%{buildroot}%{_prefix}
# FIXME: hacks for our juggler build.
rm -rf %{buildroot}%{_prefix}/share/tweek/data/omni.examples
rm -rf %{buildroot}%{_prefix}/share/tweek/test
...
%{_prefix}/lib/flagpoll/tweek-1.0.2.Linux.i386.fpc
%{_prefix}/share/aclocal/tweek.m4
%{_prefix}/share/tweek/tweek.appdefs.mk
...
%{_includedir}/tweek/idl/BeanDeliverySubject.idl
%{_includedir}/tweek/idl/Observer.idl
%{_includedir}/tweek/idl/Subject.idl
%{_includedir}/tweek/idl/SubjectManager.idl
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-JDOM.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-JGoodies-Looks.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-JacORB.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-Kunststoff.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-Liquid.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-Metouia.txt
%doc %{_prefix}/share/tweek/data/licenses/LICENSE-OpenORB.txt
%doc %{_prefix}/share/tweek/data/licenses/README-SOURCES.txt
