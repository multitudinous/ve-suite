...
export CFD_LIBS=`uname`_LIB
export VE_LIBS=`uname`
echo "CFD_LIBS =" ${CFD_LIBS}

echo "HOSTTYPE =" $HOSTTYPE
#iris4d on sgi, i386-linux on lagarto, i586-linux on abbott, i686 on cron job

case "$HOSTTYPE" in
   "") 
   #echo "HOSTTYPE is empty"
   export CFDHOSTTYPE=`uname`
;;
   ?*iris*) 
   #echo "HOSTTYPE contains iris"
   export CFDHOSTTYPE=${HOSTTYPE}
;;
   ?*linux*) 
   #echo "HOSTTYPE contains linux"
   export CFDHOSTTYPE=${HOSTTYPE}
;;
   *)
   #echo "HOSTTYPE does not contain linux or iris"
   # get here when cronjob returns HOSTTYPE = i686: switch to usual settings
   export CFDHOSTTYPE=i386-linux
;;
esac
...
#/home/vr/Juggler/irix/mipspro
   ...
   if [ ${CFDHOSTTYPE} = "i386-linux" ]; then
   ...
   elif [ ${CFDHOSTTYPE} = "i586-linux" ]; then
   ...
   echo "       CFD_LIBS and OMNI_HOME are undefined"
fi
