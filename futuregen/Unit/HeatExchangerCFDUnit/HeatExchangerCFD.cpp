...
HeatExchangerCFD::HeatExchangerCFD ()
{
  _prt_restart = 0;
  _gas_restart = 0;
...
void HeatExchangerCFD::load_and_run_glacier()
{
  /*
...
#ifndef WIN32
  if(chdir(path.c_str())) {
    cerr << "HeatExchangerCFD: empty working directory path\n";
    return ;
  }
#else
...
void HeatExchangerCFD::load_geom(int* fni,int* fnj,int* fnk,int* fnx,int* fny,int* fnz,
...
void HeatExchangerCFD::load_scirun_pd(float *pd, float *pmf, int *nps, int *numstr)
{
...
void HeatExchangerCFD::load_scirun_wics(float *wic, int *j, int *nlm, int *np, int *nel)
{
...
void HeatExchangerCFD::load_scirun_groups(float *f, float *t, float *p, int *Istage)
{
...
void HeatExchangerCFD::load_scirun_hhv(float *omegal, float *yy, float *omegaa, float *hc0)
{
...
void HeatExchangerCFD::load_scirun_coal(float *coal_flows)
{
...
void HeatExchangerCFD::send_scirun_data(int *ns, int *nlm,
...
GasCell HeatExchangerCFD::outlet_cell(int i, int j, int k, int face,
...
void HeatExchangerCFD::insert_summary_val(char *description, float *value,
				  unsigned int slen)
{
...
void HeatExchangerCFD::insert_xdata(char *xname, float *values, int *num_values,
			    unsigned int slen)
{
   // load x data entries
...
void HeatExchangerCFD::insert_ydata(char *yname, float *values, int *num_values,
...
void HeatExchangerCFD::update_plot()
{
}

//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/

void HeatExchangerCFD::load_scirun_slag(double *deltaw, double *deltar, double *kw, double *kr,
...
void HeatExchangerCFD::load_scirun_flags (int *lrsrt, int *lprst)
{
  *lrsrt = _gas_restart;
  *lprst = _prt_restart;
...
void HeatExchangerCFD::load_oxidant (float *tf0, float *hsub0, float *densf0, float *erf0, float *smf0,
...
int WIN_PREFIX gas_abort_status_()
{
  ...
  if(HX_GLACIER_PTR->get_abort_glacier()) ret=1;
  else                                     ret=0;
...
void WIN_PREFIX gas_load_geom_sr_(int* ni,int* nj,int* nk,int* nx,int* ny,int* nz,
		       float *x_vals,float *y_vals,float *z_vals,int *icell_array)
{
  HX_GLACIER_PTR->load_geom(ni,nj,nk,nx,ny,nz,x_vals,y_vals,z_vals,icell_array);
}
...
{
  HX_GLACIER_PTR->send_current_outputs(iteration_number);
}
...
{
  HX_GLACIER_PTR->send_current_outputs_nox(iteration_number);
}
...
void WIN_PREFIX gas_load_current_traj_point_(int *IJKNT, float *XP,float *YP,
				  ...
				  float *SIGMA0)
{   
  HX_GLACIER_PTR->load_current_traj_point(XP, YP, ZP,
					   TIM, PNFRP,
...
{
  HX_GLACIER_PTR->update_sr_begin_particles();
}
...
{
  HX_GLACIER_PTR->update_sr_end_particles(nix);
}
...
{
  HX_GLACIER_PTR->sr_begin_particle(ips,isl);
}
...
{
  HX_GLACIER_PTR->sr_end_particle(ips,isl,nps,nsl);
}
...
{
  HX_GLACIER_PTR->load_scirun_wics(wic, j, nlm, np, nel);
}
...
{
  HX_GLACIER_PTR->load_scirun_groups(f, t, p, istage);
}
...
{
  HX_GLACIER_PTR->load_scirun_hhv(omegal, yy, omegaa, hc0);;
}
...
{
  HX_GLACIER_PTR->load_scirun_pd(pd, pmf, nps, numstr);
}
...
{
  HX_GLACIER_PTR->load_scirun_coalMT(coal_flows, coal_temps);
}
...
{
  HX_GLACIER_PTR->load_scirun_coal(coal_flows);
}
...
void WIN_PREFIX gas_send_scirun_data_(int *ns, int *nlm,
			   ...
			   unsigned int s1len, unsigned int s2len, unsigned int s3len)
{
  HX_GLACIER_PTR->send_scirun_data(ns, nlm,
				    sns, stb, sew,
				    part_flow, part_temp, part_size, part_var,
				    u, v, w, x, y, z,
				    eff, eta, chi,
...
void WIN_PREFIX gas_send_scirun_specie_(int *ns, float *spec_val, char *spec_name,
			     unsigned int slen)
{
  HX_GLACIER_PTR->send_scirun_specie(ns, spec_val, spec_name, slen);
}
...
{
  HX_GLACIER_PTR->insert_summary_val(description, value, slen);
}
...
void WIN_PREFIX gas_insert_xdata_(char *xname, float *values, int *num_values,
		       unsigned int slen)
{
  HX_GLACIER_PTR->insert_xdata(xname, values, num_values, slen);
}
...
void WIN_PREFIX gas_insert_ydata_(char *yname, float *values, int *num_values,
		       char *xdata_name, unsigned int s1len, unsigned int s2len)
{
  HX_GLACIER_PTR->insert_ydata(yname, values, num_values, xdata_name, s1len, s2len);
}
...
void WIN_PREFIX gas_load_scirun_slag_(double *deltaw, double *deltar, double *kw, double *kr,
			   ...
			   double *ta1, double *tcv, double *emiss, double *ashcomp)
{
  HX_GLACIER_PTR->load_scirun_slag(deltaw, deltar, kw, kr,
				    ks, kd, ha, rhos,
...
{  
  HX_GLACIER_PTR->load_scirun_flags (lrsrt, lprst);
}
...
void WIN_PREFIX gas_load_oxidant_ (float *tf0, float *hsub0, float *densf0, float *erf0, float *smf0,
	  ...
	  char *spec_name, char *wic_name, unsigned int s1len, unsigned int s2len)
{
  HX_GLACIER_PTR->load_oxidant (tf0, hsub0, densf0, erf0, smf0,
