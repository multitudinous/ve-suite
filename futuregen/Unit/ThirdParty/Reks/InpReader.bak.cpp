...
{ 
  ...
  if (match(word, "TEMP"))
    ...
    return 28;                        
  else if (match(word, "T_TIME_SPEC"))
    return 29;
...
//some initialization code here to init the member variables
  ...
  do 
	...
	case 5:
	  ...
	  psr_preset = 1;
	  temp = get_specp(toks[1],speclist);
	  if (temp==NULL)
	    {
	  ...
	  cur_sp = new REKS_usr_specie(temp);
	  
	  cur_sp->set_mole_fraction(atof(toks[2].c_str()));
...
void REKS_inp_reader::premix()
{
	int i;
  
	double mmwF = 0.0;
	double mmwO = 0.0;
	double total_pos_valence_F = 0.0;
	double total_neg_valence_F = 0.0;
	double total_pos_valence_O = 0.0;
	double total_neg_valence_O = 0.0;
	double R;  
	double total_mass_F = 0.0;
	double total_mass_O = 0.0;
	REKS_usr_specie * cur_sp;
...
//calculate the mass fraction for both fuel and oxid and calc the positive and negtive valence
	...
	for (i=0; i<oxid.size(); i++)
	{	
		cur_sp = new REKS_usr_specie(oxid[i]->specie);
	  	cur_sp->set_mass_fraction(oxid[i]->mas_fraction / total_mass_F);
		cur_sp->mygas = &the_gas;
		the_gas.specs.push_back(*cur_sp);		
	}
	...
	for (i=0; i<fuel.size(); i++)
	{	
		cur_sp = new REKS_usr_specie(fuel[i]->specie);
	  	cur_sp->set_mass_fraction(fuel[i]->mas_fraction / total_mass_F);
		cur_sp->mygas = &the_gas;
		the_gas.specs.push_back(*cur_sp);		
	}
