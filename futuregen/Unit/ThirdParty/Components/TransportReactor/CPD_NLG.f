...
c
c
      subroutine cpd(Iclass,fgasx,gh2o,gco2,gch4,gco)
      IMPLICIT real*8 (A-H,O-Z)
      INCLUDE 'PARAMT.FTN'
      INCLUDE 'COMMONBL.FTN'        !zc
...
c  Output parameters for light gas species distribution in perkot4
      ...
      character*80 perkin,perkot1,perkot2,perkot3,perkot4
      character*1 ans
      dimension y(4),yp(4),ypp(4),ypred(4),tim(50),tem(50),gasf(9,10)
      dimension yygas(5),ffgas(5)
      real*8 l0,l,ma,kb,kc,kg,kp,ft(35),mt(35),mgas,mtot,mw1,mdel
      real*8 mwchar,machar,mwcharold
      integer*2 lib
c  intar = .true. calculates tar molecular weight distribution
c                 in subroutine perkp.
      logical intar,idiff,imolw,inside,yyyy
      common/cinit/l0,c0,g0,ma,rba,finf,sig,siginv,nmaxc,pstar
      common/rate/ab,eb0,ebsig,ac,ec0,ecsig,ag,eg0,egsig,rgc
      common/tarmw/u0,beta,kkk,umax,yield,prese
...
c           versions, but has not been tested on the latest versions.
      data nmaxc/10/,intar/.false./,idiff/.false./
...
c5     format(a)
      ...
      perkot3='CPDOUT3'
      perkot4='CPDOUT4'
      open(unit=1,name=perkin, status='old')
      open(unit=32,name=perkot1, status='unknown')
      open(unit=20,name=perkot2, status='unknown')
      open(unit=21,name=perkot3, status='unknown')
      open(unit=22,name=perkot4, status='unknown')
...
!zc      read(1,*)fnit
!zc      read(1,*)foxy
         fcar = COALM(1)/(1-COALM(7))
         fhyd = COALM(2)/(1-COALM(7))
         fnit = COALM(4)/(1-COALM(7))
         foxy = COALM(3)/(1-COALM(7))
...
!zc      read(1,*)ntim
!zc      do 10 i=1,ntim
!zc        read(1,*)tim(i),tem(i)
!zc        tim(i) = tim(i)/1000.
!zc10    continue
c   read time step for calculation
!zc      read(1,*)dt0,iprint,dtmax
!zc      read(1,*)timax
!zc      read(1,*)nmaxc
!zc
      nt = 1
...
1       IF(time.LE.tim(nt+1))then
     ...
     x                                  (tim(nt+1)-tim(nt))
        elseif(nt.lt.ntim)then
          ...
          go to 1
        ELSE
!zc          PRINT*,'REACHED END OF GAS TEMPERATURE CORRELATION'
        ...
        if(tp.gt.4000.)then
           print*,'     >>>>   WARNING    <<<<<'
           print*,' particle temperature too high----',tp
        endif
...
c  free radical light gas nitrogen release mechanism
     ...
     x exp(-erad/rgc/Tp)*(mwcharold-mwchar)/mwchar*
     x machar/mwchar/dt
...
C COMPONENT MASS CONSERVATION
           ...
           if(abs(dy1).lt.0.001)then
              dt = dt*2.
!zc              if(dt.lt.dtmax)print*,'at time=',time, 
!zc     x                        '  dt changed to ',dt
           elseif(abs(dy1).gt.0.02)then
              dt = 0.01/abs(dy1)*dt
!zc              print*,'at time=',time, 'dt changed to ',dt
           endif
           dt = min(dt,dtmax)
           dtn = dt
c  free radical light gas nitrogen release mechanism
...
c  update current and old mwchar
      mwcharold = mwchar
      gc = 2.*(1.-y(1)-c0)-y(2)
      mwchar = mw1-gc*mdel*sigp1/2.
c        if(y(1).lt.1.e-5)dt = dt0*10.
        fracr = 1.
        if(fmet.gt.1.e-5.and.acr.gt.0.)then
c           print*,'<main>acr,ecr,rgc,tp=',acr,ecr,rgc,tp
           ratecr = acr*exp(-ecr/rgc/tp)*fmet*dt
           fracr = 1.-ratecr/fmet
           fmet = fmet-ratecr
...
c  print out data
      ...
      if(idiff)then
          ...
          fdifc = -(fchar-fchar0)
          fchar0 = fchar
        write(32,2010)tms,tp,fcross,y(1),fdift,fdifg,fdifc
!zc        write(6,200)tms,tp,fcross,y(1),fdift,fdifg,fdifc
      else
          gasmw = rba*ma/2.
...
202   format('c time(ms)      temp     fcross     labile   ftar    ',
     x'fgas   fsolid   ftot   fmet')
        ...
        if(mod(iii-1,iprint).eq.0)then
          write(32,200)tms,tp,fcross,y(1),ftar,fgas,fchar,fvol,
     x                fmet
c          write(6,*)fcross,fmet
!zc          write(6,200)tms,tp,fcross,y(1),ftar,fgas,fchar,fvol,
!zc     x                fmet
        endif
      endif
...
!zc   ignore fother
      fgasx = fgas - ffgas(5)
      gh2o=ffgas(1)*1000.0/18.0       !mole H2O
      gco2=ffgas(2)*1000.0/44.0       !mole CO2
      gch4=ffgas(3)*1000.0/16.0       !mole CH4
      gco =ffgas(4)*1000.0/28.0       !mole CO
          intar = .false.
          fvol = fgas+ftar
          fchar = 1.-fvol
          write(32,200)tms,tp,fcross,y(1),ftar,fgas,fchar,fvol,
...
c
c     nmaxc = number of terms in expansion for mol. wt. distribution
      real*8 l0,l,ma,kb,kc,kg,kn,kp,ft(35),mt(35),y(4),yp(4)
      common/cinit/l0,c0,g0,ma,rba,finf,sig,siginv,nmaxc,pstar
      common/rate/ab,eb0,ebsig,ac,ec0,ecsig,ag,eg0,egsig,rgc
      common/nitmod/fnca0,fnca,an,en0,ensig
      data fx/0./
...
c
      real*8 l0,l,ma,mb,kb,kc,kg,kp,y(3),yp(3),ft(35),mt(35),
...
!zc         if(iprint.eq.1)then
!zc           if(n.eq.1)then
!zc             print*,'Initial Fragment size distribution'
!zc             print*,'n   fraction      Cumul.   MW'
!zc           Endif
!zc           print*,n,ft(n),ftsum,mt(n)
...
c
     ...
     x                  temp,prese,nmaxc,imolw,sumy)
c      IMPLICIT real*8 (A-H,O-Z)
      save
c  flash distillation of metaplast to form liquid and tar vapor
      logical imolw
      real*8 k(36),l(36),Ltot,mt(35),metold(35)
...
250     continue
        ...
        if(imolw)then
          ip = ip+1
!zc          print*,'Weight Av. Molecular Wt.=',xmwtot
!zc     x         ,' Gas MW =',gasmw
        endif
