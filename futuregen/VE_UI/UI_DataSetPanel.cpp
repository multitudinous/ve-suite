...
{ 
   ...
   //new layout
   _col1->Add(_3dRBox,1,wxEXPAND|wxALIGN_CENTER_HORIZONTAL|wxHSCROLL);
   _col1->Add(_polydataRBox,1,wxEXPAND|wxALIGN_CENTER_HORIZONTAL|wxHSCROLL);
   _col1->Add(_vertexRBox,1,wxEXPAND|wxALIGN_CENTER_HORIZONTAL|wxHSCROLL); 
...
{
   _numSteadyStateDataSets = ((UI_Frame*)GetParent())->_tabs->datasetNum;
   ...
   if (_numSteadyStateDataSets > 0)
   {
      for (CORBA::ULong i = 0; i<_numSteadyStateDataSets; i++)
      {
         ...
         thisDataSet->_dataSetName = ((UI_Frame*)GetParent())->_tabs->datasetNames[i];
         thisDataSet->_dataSetType = ((UI_Frame*)GetParent())->_tabs->datasetTypes[i];
         
         wxString scalarNames[((UI_Frame*)GetParent())->_tabs->numScalarsPerDataset[i]];
         
         for (int k=0; k<((UI_Frame*)GetParent())->_tabs->numScalarsPerDataset[i]; k++)
         {
            scalarNames[k] = ((UI_Frame*)GetParent())->_tabs->sc_attrib[index];
            index++;
         }
         
         thisDataSet->_buildScalars(((UI_Frame*)GetParent())->_tabs->numScalarsPerDataset[i], scalarNames);
         
         _DataSets.push_back(thisDataSet);

         //clean up the names array
         for(int p = ((UI_Frame*)GetParent())->_tabs->numScalarsPerDataset[i] -1; p >= 0; p--)
         {
