...
static const char* vrPhongFragSource = {
   ...
   "{\n"
      ...
      "   float RDotL=max(dot(R,L),0.0); \n"
         ...
         "vec3 TotalAmbient=gl_LightSource[0].ambient.rgb*vec3(.3,.3,.3)*textureColor.rgb; \n"
         "vec3 TotalDiffuse=gl_LightSource[0].diffuse.rgb*textureColor.rgb*NDotL; \n"
         "vec3 TotalSpecular=gl_LightSource[0].specular.rgb*vec3(1.0,1.0,1.0)*pow(RDotL,50.0); \n"
         "gl_FragColor =   vec4(TotalDiffuse+TotalSpecular+TotalAmbient,textureColor.a);\n"
      "//}else\n"
      "{\n"
