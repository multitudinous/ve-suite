...
public:
    ...
    unsigned char* dataField( int timeStep )
    {
        return _dataFields.at( timeStep );
    }
    ...
    int numberOfFields()
    {
        return static_cast< int >( _dataFields.size() );
    }
    ...
    ScalarRange dataRange( int index )
    {
        return _ranges.at( index );
    }

    ///The scalar range for the entire sequence
    float* transientRange()
    {
        return _transientRange;
    }
    ...
    DataType GetDataType( int whichField )
    {
        return _types.at( whichField );
    }
