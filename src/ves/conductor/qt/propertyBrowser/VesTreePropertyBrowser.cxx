...
#include <ves/conductor/qt/propertyBrowser/VesTreePropertyBrowser.h>
...
VesTreePropertyBrowser::VesTreePropertyBrowser(QWidget* parent) :
    ...
    mParser( 0 )
{
    ...
    mFileEditFactory = new propertystore::ExternalStringSelectFactory(this);
    // This picks up the ves-specialized version of FileEdit
    mFileEditFactory->setEditorType( new FileEdit );

    mNodeEditFactory = new propertystore::ExternalStringSelectFactory( this );
    mNodeEditFactory->setEditorType( new NodeEdit );
}

VesTreePropertyBrowser::~VesTreePropertyBrowser()
{
    delete mParser;
}

void VesTreePropertyBrowser::ParsePropertySet( propertystore::PropertySetPtr set, bool autosize )
{
    if( !mParser )
    {
        SetPropertyParser( new propertystore::PropertyParser );
    }
...
void VesTreePropertyBrowser::SetPropertyParser( propertystore::PropertyParser* parser )
{
    if( mParser )
    {
        delete mParser;
    ...
    propertystore::ExternalStringSelectManager* filePathManager = new propertystore::ExternalStringSelectManager;
    ...
    propertystore::ExternalStringSelectManager* nodePathManager = new propertystore::ExternalStringSelectManager;
    parser->AddCustomExternalStringManager( nodePathManager, "isNodePath" );
    this->setFactoryForManager( nodePathManager, mNodeEditFactory );
}

void VesTreePropertyBrowser::RefreshContents( bool autosize )
{
    ...
    propertystore::PropertyParser::ItemVector* items = mParser->GetItems();
    propertystore::PropertyParser::ItemVector::iterator iterator;
    propertystore::PropertyParser::ItemVector::iterator end = items->end();
...
void VesTreePropertyBrowser::RefreshValues()
{
    if( mParser )
    {
        mParser->Refresh();
...
void VesTreePropertyBrowser::RefreshAllValues()
{
    if( mParser )
    {
        mParser->RefreshAll();
